...
import PrintLabelPlastic from 'pages/PrintLabelPlastic';
...
class Spdo extends React.Component {
	...
	connectWebSocket = async (context) => {
		var socket = new WebSocket(`ws://10.0.111.143:4444/CHCDO/scanBarcodeMessage`);
	...
	updateDetailSPOnMessage = async (message) => {
		if (message.body) {
			...
			this.getDetailSPRefresh(outcode, noSP, group, userID, lapakYN, paymentMethod);
		}
	}               
	...
	getProductListToSend = async (listProduct) => {
		...
		console.log('listProcodToSend: ' + JSON.stringify(listProcodToSend));
		this.getProductList();
	}

	getProductList = async () => {
		var url = `http://10.0.111.212:8092/CHCMasterProduk/Product/Custom?find=barcode&by=procodes`;
		console.log(url);

		var payload = {
			procodes: listProcodToSend
		}
		console.log('listProcodToSendPayload: ' + JSON.stringify(payload));
	...
	//---------------------------------------------------------
	clickRowSPList(SPID, outcodeDari, outcode, noSP, group, userID, outlet, lapakYN, paymentMethod) {
		...
		this.getDetailSPFirst(SPID, outcode, noSP, group, userID, lapakYN, paymentMethod)
	}

	async getDetailSPFirst(SPID, outcode, noSP, group, userID, lapakYN, paymentMethod) {
		tableListSPOnPage = false;
	...
	async getDetailSPRefresh(outcode, noSP, group, userID, lapakYN, paymentMethod) {
		if (detailSPOnPage) {
			tableListSPOnPage = false;
	...
	saveEditProduct() {
		...
		console.log('saveEditProduct: ' + JSON.stringify(listDetailDO));
		console.log('saveEditProduct: ' + JSON.stringify(listBatchDetail));
	...
	handleScanOnClick() {
		...
		console.log('ScanClick: listProcod: ' + listProcod);
		console.log('ScanClick: listBarcode: ' + listBarcode);
	}
	...
	scanInputValidate() {
		if (listBarcode.length === 0 || listProcod.length === 0) {
			console.log('ListBarcode: ' + JSON.stringify(listBarcode));
			console.log('ListProcod: ' + JSON.stringify(listProcod));
			return;
		}
	...
	openEditBatchModal(productIndex, noDO, outcodeTransf, procod, prodes, batchLama, qtySP, ED, noOrder, categoryProduct, medunit) {
		...
		qtyLamaConverted = qtyLamaConverted.includes('(') ? qtyLamaConverted.substr(0, qtyLamaConverted.indexOf('(')).replace(' ', '') : qtyLamaConverted;
	...
	async editBatchSave() {
		...
		let url = `${BACKEND_HOST_URL}:${BACKEND_HOST_PORT}/${BACKEND_HOST_PREFIX_DO}/UpdateBatch`
		console.log(url);
	...
	render() {
		return (
			<Page
				title="SP DO"
				breadcrumbs={[{ name: 'Delivery Order', active: false }, { name: 'SP DO', active: true }]}>
				<Card className="mb-3">
					<NotificationSystem
						dismissible={false}
										...
										<Col md={6} xs={6}>
											<InputGroup>
												<InputGroupAddon addonType='prepend'>
													<Button color='info' onClick={() => this.handleScanOnClick()}><MdBorderHorizontal /> SCAN QR</Button>
												</InputGroupAddon>
												<Input
												...
												<InputGroupAddon addonType='append'>
													<InputGroupText>{this.state.scanInputValue}</InputGroupText>
												</InputGroupAddon>
											</InputGroup>
										</Col>
									</Row>
				...
				<Modal
					...
					<ModalBody>
						<Row><Col><Label>Scan QR Code di bawah dengan Smartphone Anda</Label></Col></Row>
						<Row>
							<Col>
								<a href={this.state.qrLink} target="_blank" className='w-100 d-flex justify-content-center'>
									<QRCode size={200} value={this.state.qrLink} />
								</a>
							</Col>
						</Row>
					</ModalBody>
					<ModalFooter><Row><Col><Button color='danger' className='ml-auto' onClick={this.toggle('scanQRCode')}>Close</Button></Col></Row></ModalFooter>
				</Modal>
				...
				<Modal
					...
					<ModalFooter>
						<Button color='success'  onClick={() => this.confirmDetailSP()} ><MdDone /> Ya</Button>
