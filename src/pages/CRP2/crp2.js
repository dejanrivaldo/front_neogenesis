...
const GET_NEXT_CLUSTER_INTERVAL = 30;
...
class CRP2Page extends React.Component {
    ...
    componentDidMount() {
        ...
        setInterval(() => {
            ...
            this.setState({
                clock: new Date(),
                nextClusterTimer: currentClusterTimer - 1 != 0 ? currentClusterTimer - 1 : GET_NEXT_CLUSTER_INTERVAL
            })

            if(currentClusterTimer - 1 == 0) {
                this.getNextClusterNotRefillList();
            }
        }, 1000); //Clock interval function every second
    }
    ...
    getNotRefillList(cluster, tanggalAwal, tanggalAkhir) {
        ...
        const url = `${BACKEND_HOST_URL}:${BACKEND_HOST_PORT}/CRP2/${db}?tanggalAwalSP=${tanggalAwal}&tanggalAkhirSP=${tanggalAkhir}`;
                ...
                <Card className="mb-3">
                    ...
                    <CardBody>
                        <Card outline color="primary">
                            <Form className='mt-4'>
                                <FormGroup row className='d-flex justify-content-center align-items-center'>
                                    ...
                                    <Label md={3}>will refresh in {this.state.nextClusterTimer} seconds</Label>
                                </FormGroup>
                                <FormGroup row className='d-flex justify-content-center align-items-center'>
                                    <Col md={2}>
                        ...
                        <Row className='d-flex justify-content-center'>
                            ...
                            <Label className={(this.state.isLoading || this.state.notRefillList.length > 0 ? ' d-none ' : ' ')}>Tidak ada data</Label>
                        </Row>
                    </CardBody>
                </Card>
            </Page>
