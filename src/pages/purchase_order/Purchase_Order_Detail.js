...
var listProcodDeleteOutlet = [];
var listProcodRemoveOutlet = [];
...
// =========================================================================
  handleProcodOutletClick(event, detail, index) {
      console.log('procod outlet clicked')
    const checked = event.target.checked;
    var procodCheckList = this.state.procodCheckList;

    if (checked === true) {
      listProcodRemoveOutlet.push(detail['POD_Procod']);
      listProcodDeleteOutlet.push(detail['POD_Procod']);
      procodCheckList[index] = true;
      this.setState({
        procodCheckList: procodCheckList
      });
    } else {
        listProcodDeleteOutlet.splice(listProcodDeleteOutlet.indexOf(detail), 1);
        listProcodRemoveOutlet.splice(listProcodRemoveOutlet.indexOf(detail), 1);
      procodCheckList[index] = false;
      this.setState({
        procodCheckList: procodCheckList,
  ...
  ButtonClickDeleteOutlet = async () => {
    console.log('procod outlet delete clicked')
    var currListProcodDelete = this.state.ListProcod;
    this.setState(
      {
        arrayListProcodOutletRemove: currListProcodDelete.concat(listProcodDeleteOutlet),
        modal_procodeSearch: false,
      });
  };
  ButtonClickRemoveOutlet = async () => {
    console.log('procod outlet remove clicked')
    var currListProcodRemove = this.state.ListProcod;
    this.setState(
      {
    arrayListProcodOutletRemove: currListProcodRemove.concat(listProcodRemoveOutlet),
        modal_procodeSearch: false,
      });
  };
            ...
            className   = "Detail">
                ...
                <Table
                    ...
                    responsive>
                        ...
                        <tbody>
                            {this.state.resultPODetail2.map((detail,index) =>
                                <tr>   
                                    <Col>
                                    
                                    <Input
                                    checked={this.state.procodCheckList[index]}
                                    value={this.state.checkedBrand}
                                    onClick={event =>
                                      this.handleProcodOutletClick(event, detail, index)
                                    }
                                    >
                ...
                <Row >
                    <ButtonGroup  size="sm" className = {"ml-3"}>
                        ...
                        onClick={() =>
                            this.ButtonClickDeleteOutlet()
                          }
                        >Delete Outlet</Button>
                        <Button variant="secondary"
                        onClick={() =>
                            this.ButtonClickRemoveOutlet()
                          }
